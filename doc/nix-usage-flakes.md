...
```
    ...
    * packages.x86_64-linux.bluos-controller
      BluOS Controller 4.12.0 (non-free)
...
As discussed in prior sections, the positional arguments to `nix build` are *installables*. Here, the `.` indicates that our flake should be found from the current directory. Within this flake, we look for a package with an attribute name of `bluos-controller`. We didn't have to use the full attribute path `packages.x86_64-linux.bluos-controller` because `nix build` will automatically look in the `packages` attribute for the system it detects we're on.
...
```
    ...
    /nix/store/4svavnl9zkgy409bzc835pc2vhw0vygg-bluos-controller
...
```
    ...
    {
      "packages.x86_64-linux.bluos-controller": {
        "description": "BluOS Controller 4.12.0 (non-free)",
...
```
    ...
    /nix/store/4svavnl9zkgy409bzc835pc2vhw0vygg-bluos-controller/bin/bluos-controller
...
```
    ...
    Store paths:        /nix/store/4svavnl9zkgy409bzc835pc2vhw0vygg-bluos-controller
