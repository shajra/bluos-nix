...
|------------------------------------ |------------------------------------------ |
| `<output attribute>.<system>.<name>` | `.#packages.x86_64-linux.bluos-controller` |
...
```
    ...
    * packages.x86_64-linux.bluos-controller
      BluOS Controller 3.20.6 (non-free)
...
```
    ...
    * legacyPackages.x86_64-linux.kitty (0.30.0)
...
```sh
nix build .#bluos-controller
```

The positional arguments to `nix build` are *installables* as discussed in prior sections. Here, the `.` indicates that our flake should be found from the current directory. Within this flake we look for a package with an attribute name of `bluos-controller`. We didn't have to use the full attribute path `packages.x86_64-linux.bluos-controller` because `nix build` will automatically look in the `packages` attribute for the system it detects we're on.
...
```
    ...
    /nix/store/4dac0w8314mg0g1lq95nvjiyly686vg2-bluos-controller
...
For example, to get the help message for the `bluos-controller` executable provided by the package selected by the `bluos-controller` attribute path output by this project's flake, we can call the following:
...
```
    ...
    {
      "packages.x86_64-linux.bluos-controller": {
        "description": "BluOS Controller 3.20.6 (non-free)",
...
`nix run` works because the package selected by the output attribute name `bluos-controller` selects a package with a package name “bluos-controller” that is the same as the executable provided at `bin/bluos-controller`.
...
In the examples above, we've used selected packages from this project's flake, like `.#bluos-controller`. But one benefit of flakes is that we can refer to remote flakes just as easily, like `nixpkgs#hello`. This means we can build quickly build environments with `nix shell` or run commands with `nix run` without committing to install software.
...
```
    ...
    /nix/store/4dac0w8314mg0g1lq95nvjiyly686vg2-bluos-controller/bin/bluos-controller
...
To install the `bluos-controller` executable, which is provided by the `bluos-controller` output of our flake, we'd run the following:

```sh
nix profile install .#bluos-controller
```
...
```
    ...
    0 git+file:///home/tnks/src/shajra/bluos-nix#packages.x86_64-linux.bluos-controller git+file:///home/tnks/src/shajra/bluos-nix#packages.x86_64-linux.bluos-controller /nix/store/4dac0w8314mg0g1lq95nvjiyly686vg2-bluos-controller
