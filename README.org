...
application, which lends to portability, which we get with light patching.
...
The controller is wrapped by =daemon= as a convenience for job control, ensuring
that only one instance runs at a time. Also, you don't have to deal with
backgrounding processes or redirecting standard output/error.
